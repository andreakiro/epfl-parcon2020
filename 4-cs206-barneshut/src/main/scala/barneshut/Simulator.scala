...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics) {
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries = {
    ...
    resultBoundaries.minY = Math.min(boundaries.minY, body.y)
    resultBoundaries.minX = Math.min(boundaries.minX, body.x)
    resultBoundaries.maxY = Math.max(boundaries.maxY, body.y)
    resultBoundaries.maxX = Math.max(boundaries.maxX, body.x)
