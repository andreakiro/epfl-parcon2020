...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    grouped.map(
      tuple => {
        val questions: Posting = tuple._2.head._1
        val answers: Array[Posting] = tuple._2.unzip._2.toArray
        (questions, answerHighScore(answers))
      }
    )
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(
      (questiontoscore: (Posting, Int)) => 
        (firstLangInTag(questiontoscore._1.tags, langs).get * langSpread, questiontoscore._2)
    ).persist()
  }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    val clusterVectors: Map[Int, Iterable[(Int, Int)]] = vectors.map(v => (findClosest(v, means), v)).groupByKey.collect.toMap
    for (i <- newMeans.indices; if clusterVectors.get(i).isDefined) 
      newMeans(i) = averageVectors(clusterVectors(i))
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langPercent: Double =
        ...
        (100.0 * languages.size.toDouble) / vs.size.toDouble // percent of the questions in the most common language
